...
public class PercentExpense implements Expense{
    ...
    public void executeExpense(String payingUserId, Double amount, int numOfUsers, List<String> usersInvolved,User payingUser,
        ...
        List<Double> balanceList = getBalanceList(numOfUsers, expenseSplit, amount);
        updateExpenseBalance(payingUser, usersInvolved, balanceList, userBalanceMap);
    }


    List<Double> getBalanceList(int numOfUsers, String[] expenseSplit, Double amount) {
        List<Double> balanceList = new ArrayList<>();
        for(int i =1;i<=numOfUsers;i++) {
            Double calculateBalance = amount * Double.valueOf((Double.valueOf(expenseSplit[3 + numOfUsers + 1 +i]))/100);
            balanceList.add(calculateBalance);
        }
        return balanceList;
    }
